...
with tab1:
    ...
    if df.empty:
        ...
        # --- Unit viewing option: distinguish Unit 1 vs Unit 2 vs Common ---
        df['unit_category'] = df['system_kks'].apply(get_unit_from_kks) if 'system_kks' in df.columns else "Unknown"

        st.markdown("#### Viewing scope")
        unit_view = st.radio(
            "Unit",
            options=["All Units", "Unit 1", "Unit 2", "Common/Shared", "Temporary"],
            horizontal=True,
            key="analytics_unit_view",
            help="Filters every metric, chart, and table below by which unit the KKS code belongs to "
                 "(derived from the 2-digit unit prefix — 1x=Unit 1, 2x=Unit 2, 0x=Common/Shared, 9x=Temporary)."
        )
        if unit_view != "All Units":
            df = df[df['unit_category'] == unit_view]

        if df.empty:
            st.warning(f"No records found for {unit_view}.")
            st.stop()

        # De-duplicated counts: count UNIQUE system_kks values per scope type,
        # not raw row counts — the same building/system/equipment can
        # legitimately appear in more than one row (e.g. re-imported from a
        # different source file, or logged again after edits), and counting
        # every row would overstate how many distinct items are tracked.
        def unique_count(scope_type: str) -> int:
            if 'scope_type' not in df.columns or 'system_kks' not in df.columns:
                return 0
            subset = df[df['scope_type'] == scope_type]
            return subset['system_kks'].nunique()

        # --- Top Metrics Row ---
        col1, col2, col3, col4, col5 = st.columns(5)

        with col1:
            st.metric("Systems Tracked", unique_count('System'), help="Unique KKS codes, duplicates not counted")

        with col2:
            st.metric("Equipment Tracked", unique_count('Equipment'), help="Unique KKS codes, duplicates not counted")

        with col3:
            st.metric("Buildings Tracked", unique_count('Building'), help="Unique KKS codes, duplicates not counted")
        ...
        # --- Unit Breakdown (only shown in "All Units" view) ---
        if unit_view == "All Units" and 'unit_category' in df.columns:
            st.subheader("Unit 1 vs Unit 2 Breakdown")
            unit_counts = df['unit_category'].value_counts().reset_index()
            unit_counts.columns = ['Unit', 'Record Count']
            ucol1, ucol2 = st.columns([1, 2])
            with ucol1:
                st.dataframe(unit_counts, width="stretch", hide_index=True)
            with ucol2:
                st.bar_chart(unit_counts.set_index('Unit'), width="stretch", height=250)
            st.markdown("---")
        ...
        for idx, ms in enumerate(MILESTONES):
            ...
            with milestone_pill_cols[idx]:
                if st.button(f"{short_label}\n{completed_n}/{len(df)}", key=f"milestone_pill_{ms}", width="stretch"):
                    st.session_state.analytics_selected_milestone = ms

        selected_ms = st.session_state.analytics_selected_milestone
        if selected_ms and selected_ms in df.columns:
            ms_label_full = MILESTONE_LABELS.get(selected_ms, selected_ms)
            completed_df = df[df[selected_ms] == 'Completed']
            not_completed_df = df[df[selected_ms] != 'Completed']

            st.markdown(f"##### {ms_label_full} — {len(completed_df)} of {len(df)} records fully complete")

            show_tab_complete, show_tab_remaining = st.tabs([
                f"Completed ({len(completed_df)})", f"Not Yet Completed ({len(not_completed_df)})"
            ])
            display_cols = [c for c in ['system', 'system_kks', 'scope_type', 'component',
                                          'commissioning_stage', 'unit_category', selected_ms, 'comments']
                             if c in df.columns]
            with show_tab_complete:
                st.dataframe(completed_df[display_cols], width="stretch", hide_index=True)
            with show_tab_remaining:
                st.dataframe(not_completed_df[display_cols], width="stretch", hide_index=True)

            if st.button("Clear selection", key="clear_milestone_selection"):
                st.session_state.analytics_selected_milestone = None
                st.rerun()
        ...
        display_cols = ['system', 'system_kks', 'unit_category', 'scope_type', 'component'] + \
...
with tab3:
    ...
    **KKS Structure** (mandatory 2-digit Unit prefix on every code):
    ...
    with st.form("manual_update", clear_on_submit=False):
        ...
        st.markdown("*IT, PIC, HT, PT, SAW are physical commissioning tests. P-1/P-2/P-3 track protocol document sign-off. All apply to every record.*")
        ...
        for idx, ms in enumerate(MILESTONES):
            ms_label = get_label(MILESTONE_LABELS, ms)
            with milestone_cols[idx % 3]:
                current = prefill.get(ms, 'Pending')
                milestone_values[ms] = st.selectbox(
                    ms_label,
                    status_options,
        ...
        it_stat = milestone_values["it_status"]
        pic_stat = milestone_values["pic_status"]
        ht_stat = milestone_values["ht_status"]
        pt_stat = milestone_values["pt_status"]
        saw_stat = milestone_values["saw_status"]
                ...
                if not valid:
                    ...
                    if stage_input and not normalized_stage:
                        st.warning(
                            ...
                            f"(a letter, optionally with a dash and number, e.g. 'A', 'A-1', 'A-3.1') — saved as-is."
                        )
                    record = {
                        "system": sys_name,
                        "system_kks": kks_code,
...
with tab5:
    ...
    if df.empty:
        ...
        column_config = {}
        editable_cols = ['system', 'system_kks', 'component', 'commissioning_stage', 'it_status', 'pic_status', 'ht_status', 'pt_status', 'saw_status', 'p1_status', 'p2_status', 'p3_status', 'comments']
